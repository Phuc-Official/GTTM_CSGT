...
class VehiclePlatnoButton extends StatefulWidget {
  ...
  const VehiclePlatnoButton({Key? key, required this.onVehicleSelected})
      : super(key: key);

  @override
  _VehiclePlatnoState createState() => _VehiclePlatnoState();
}

class _VehiclePlatnoState extends State<VehiclePlatnoButton> {
  int? selectedIndex;
  ...
  Future<void> _loadPlateNumbers() async {
    final vehicleService =
        VehicleService('http://192.168.105.250:4113/traffic-violation');
    try {
          ...
          children: vehicles.asMap().entries.map((entry) {
            ...
            return _buildButton(
              ...
              () {
                ...
                widget.onVehicleSelected; // Gọi callback với logId
              },
            );
          }).toList(),
        ),
  ...
  Widget _buildButton(
      int index, String iconPath, String label, VoidCallback onPressed) {
    ...
    return Padding(
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(
          backgroundColor:
              isSelected ? AppColors.blueText : AppColors.bgVehicle,
